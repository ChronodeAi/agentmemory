...
describe("ClipEmbeddingProvider (with loaded pipeline)", () => {
  ...
  it("preserves local processing metadata", async () => {
    mockSuccessModule();
    const { ClipEmbeddingProvider: Fresh } = await import(
      "../src/providers/embedding/clip.js"
    );

    expect(new Fresh().processingLocation).toBe("local");
  });
  ...
  it("loads image pipeline with dtype: q8 and decodes data URLs", async () => {
