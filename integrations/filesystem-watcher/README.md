...
Session id and project are required by the observe endpoint. Set them via env, or the watcher creates one session per root and derives the project from the root's credential-free Git remote. Repositories without a remote use a hashed canonical-path identity.
