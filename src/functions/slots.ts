...
function scopeKv(scope: SlotScope, project?: string): string {
  ...
  if (!project) throw new Error("project is required for project-scoped slots");
...
async function readSlot(
  ...
  const projectSlot = await kv.get<MemorySlot>(
...
export async function listPinnedSlots(
  ...
  await seedDefaults(kv, project);
  const [projectSlots, global] = await Promise.all([
    kv.list<MemorySlot>(KV.projectSlots(project)),
...
export function registerSlotsFunctions(sdk: ISdk, kv: StateKV): void {
  ...
  sdk.registerFunction("mem::slot-list", async (data: { project?: string }) => {
    ...
    if (!project) return { success: false, error: "project required" };
    await seedDefaults(kv, project);
    const [projectSlots, global] = await Promise.all([
      kv.list<MemorySlot>(KV.projectSlots(project)),
      kv.list<MemorySlot>(KV.globalSlots),
  ...
  sdk.registerFunction(
    ...
    async (data: {
      ...
      if (scope === "project" && !project) {
        return { success: false, error: "project required for project scope" };
      }
      ...
      const resolvedSizeLimit = sizeLimit ?? DEFAULT_SIZE_LIMIT;
      const content = typeof data?.content === "string" ? data.content : "";
      if (content.length > resolvedSizeLimit) {
        return { success: false, error: `content exceeds sizeLimit (${content.length} > ${resolvedSizeLimit})` };
      }
      const description = typeof data?.description === "string" ? data.description : "";
      const pinned = typeof data?.pinned === "boolean" ? data.pinned : true;
      return withKeyedLock(`slot:${project ?? "global"}:${label}`, async () => {
        // Duplicate check is scope-local so a project slot can shadow a
        // global slot with the same label — matches the read precedence.
        const existing = await readSlotInScope(kv, label, scope, project ?? undefined);
        ...
        await kv.set(scopeKv(scope, project ?? undefined), label, slot);
  ...
  sdk.registerFunction(
    ...
    async (data: { label?: string; text?: string; project?: string }) => {
      ...
      return withKeyedLock(`slot:${project}:${label}`, async () => {
        const { slot, scope } = await readSlot(kv, label, project);
  ...
  sdk.registerFunction(
    ...
    async (data: {
      ...
      const project = validateProject(data.project) ?? session?.project ?? null;
      if (!project) return { success: false, error: "project required" };
      ...
      if (pendingLines.length > 0) {
        const pendingApplied = await withKeyedLock(`slot:${project}:pending_items`, async () => {
