...
| `memory_context_packet` |  | `project`*: string, `sessionId`*: string, `query`: string, `files`: string, `token_budget`: number | Build the project-scoped Recall packet for a coding task with fixed budgets for identity, lessons, episodic history, file history, and provenance. |
